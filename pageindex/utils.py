...
PAGEINDEX_LLM_PROVIDER = os.getenv("PAGEINDEX_LLM_PROVIDER", "openai").strip().lower()
...
def _get_provider():
    ...
    provider = provider_aliases.get(PAGEINDEX_LLM_PROVIDER)
    if provider is None:
        raise ValueError(
            f"Unsupported PAGEINDEX_LLM_PROVIDER: {PAGEINDEX_LLM_PROVIDER}"
        )
    return provider


def _create_openai_client(api_key=None):
    provider = _get_provider()
    if provider == "azure":
        azure_api_key = api_key or AZURE_OPENAI_API_KEY
        if not AZURE_OPENAI_ENDPOINT:
            raise ValueError(
                "AZURE_OPENAI_ENDPOINT is required when PAGEINDEX_LLM_PROVIDER=azure"
            )
        if not azure_api_key:
            raise ValueError(
                "AZURE_OPENAI_API_KEY is required when PAGEINDEX_LLM_PROVIDER=azure"
            )
    ...
    openai_api_key = api_key or CHATGPT_API_KEY or OPENAI_API_KEY
    if not openai_api_key:
        raise ValueError(
            "CHATGPT_API_KEY or OPENAI_API_KEY is required when PAGEINDEX_LLM_PROVIDER=openai"
        )
    return openai.OpenAI(api_key=openai_api_key)


def _create_async_openai_client(api_key=None):
    provider = _get_provider()
    if provider == "azure":
        azure_api_key = api_key or AZURE_OPENAI_API_KEY
        if not AZURE_OPENAI_ENDPOINT:
...
async def ChatGPT_API_async(model, prompt, api_key=None):
    ...
    for i in range(max_retries):
        try:
            async with _create_async_openai_client(api_key=api_key) as client:
